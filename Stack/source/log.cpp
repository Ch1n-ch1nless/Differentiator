...
FILE* OpenLogFile(const char* FILE_NAME, const char* mode)
{
    assert(FILE_NAME);
    assert(mode);
    ...
    char* file_name = (char*) calloc(strlen(FILE_NAME) + strlen(LOG_EXTENSION) + 1, sizeof(char));
    ...
    file_name = strcat(file_name, LOG_EXTENSION);
