...
{
    //Make file
    FILE* LOG_FILE_PTR = OpenLogFile("error", "a");
...
hash_t CalculateStkHash(const Stack* stk)
{
    ...
    hash = hash * 33 + *(const hash_t* const)&(stk->data);

    for (size_t i = 0; i < stk->capacity * sizeof(stk_elem_t) + 2 * sizeof(canary_t); i++)
    {
    ...
    hash = hash * 33 + *(const hash_t* const)(&(stk->name));
    hash = hash * 33 + *(const hash_t* const)(&(stk->file));
